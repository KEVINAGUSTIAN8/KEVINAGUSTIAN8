<div align="center">

# 👋 Hi, I'm Kevin Agustian

### 💻 Beginner Web Developer &nbsp;|&nbsp; 🇮🇩 Indonesia

> *"Starting from zero, growing every day — one line of code at a time."* 🚀

</div>

---

## 🌟 About Me

```javascript
const kevin = {
  name: "Kevin Agustian",
  location: "Indonesia 🇮🇩",
  role: "Beginner Web Developer",
  currentlyLearning: ["JavaScript", "Web Development"],
  focus: ["Frontend", "Backend Basics"],
  goal: "Become a Professional Full-Stack Developer 🎯",
  motto: "Starting from zero, growing every day 💪"
};
```

---

## 🛠️ Tech Stack & Tools

<div align="center">

### Languages
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)

### Tools
![VS Code](https://img.shields.io/badge/VS%20Code-0078D4?style=for-the-badge&logo=visual%20studio%20code&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)

</div>

---

## 📊 Skill Progress

| Skill | Level | Progress |
|-------|-------|----------|
| HTML | Beginner | ███░░░░░░░ 30% |
| CSS | Beginner | ███░░░░░░░ 30% |
| JavaScript | Beginner | ██░░░░░░░░ 20% |
| PHP | Beginner | ██░░░░░░░░ 20% |
| Git | Beginner | ██░░░░░░░░ 20% |

---

## 📂 Projects

<div align="center">

| 🗂️ Project | 📝 Description | 🔧 Tech Used |
|------------|----------------|--------------|
| 🌐 Simple Website Layout | Responsive static website layout | HTML, CSS |
| 🗃️ Basic CRUD with PHP | Create, Read, Update, Delete operations | PHP, HTML |
| ⚡ JavaScript Practice | Basic DOM manipulation & logic | JavaScript |
| 🔀 Git Version Control | Version control workflow practice | Git, GitHub |

> 🚀 **More projects coming soon...**

</div>

---

## 📈 GitHub Stats

<div align="center">

<img height="160" src="https://github-readme-stats.vercel.app/api?username=KEVINAGUSTIAN8&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true" />
<img height="160" src="https://github-readme-stats.vercel.app/api/top-langs/?username=KEVINAGUSTIAN8&layout=compact&langs_count=6&theme=tokyonight&hide_border=true" />

</div>

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=KEVINAGUSTIAN8&theme=tokyonight&hide_border=true)](https://git.io/streak-stats)

</div>

---

## 🎯 My Roadmap

```
[🔄] Learn HTML — Just Getting Started
[🔄] Learn CSS — Just Getting Started
[🔄] Learn JavaScript — Just Getting Started
[🔄] Learn PHP — Just Getting Started
[🔄] Practice Git & GitHub — Just Getting Started
[⏳] Build Real Projects
[⏳] Learn a JS Framework (React / Vue)
[⏳] Build Full-Stack Projects
[⏳] Become a Professional Developer 🚀
```

---

## 📫 Connect With Me

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-KEVINAGUSTIAN8-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/KEVINAGUSTIAN8)
[![Email](https://img.shields.io/badge/Email-kevinagustian001@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kevinagustian001@gmail.com)

</div>

---

<div align="center">

---

### ⭐ Thanks for visiting my profile!

*"I may be starting from zero, but I have the determination to grow and a mentor to guide me."* 🚀

**Keep coding, keep growing! 💙**

</div>
